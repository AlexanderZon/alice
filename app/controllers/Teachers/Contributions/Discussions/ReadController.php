<?php namespace Teachers\Contributions\Discussions;
...
class ReadController extends \Teachers\Contributions\ReadController {
	...
	public function postComments( $course_id = '' ){
		...
		if($parent->id != $discussion->id):
			...
			\Event::fire('notification.discussions_write_comment', array(Auth::user(), $discussion, $comment));
	...
	public function postLike( $course_id = '' ){
		...
		if($my_thumbsup = $comment->hasThumbsup($user)):
			...
			\Event::fire('notification.discussions_like_comment', array(Auth::user(), $comment));
	...
	public function postBanned( $course_id = '' ){
		...
		if($comment->isBanned()):
			...
			\Event::fire('notification.discussions_banned_comment', array(Auth::user(), $comment));
