									...
									<div class="row">
										<div class="col-md-8">
											<div class="form-group">
												...
												<div class="col-md-7">
													<input name="title" type="text" class="form-control" placeholder="Ingrese el nombre de la Lección" value="{{ $lesson->title }}" required>
										...
										<div class="col-md-4">
											<div class="col-md-12">
												<input name="status" value="active" type="checkbox" class="make-switch" data-on-text="&nbsp;Activo&nbsp;&nbsp;" data-off-text="&nbsp;Inactivo&nbsp;" {{ ($lesson->status == 'active') ? 'checked="checked"' : '' }}>
												<!-- <span class="help-block">This is inline help</span> -->
											</div>
										</div>
										<!--/span-->
									...
									<div class="row">
										<div class="col-md-8">
											<div class="form-group">
												...
												<div class="col-md-7">
													<input id="approval_percentage" type="text" name="approval_percentage" value="{{ $lesson->approval_percentage * 100 }}"/>
												</div>
											</div>
										</div>
									</div>
															...
															<select name="previous_id" class="form-control select2me" data-placeholder="Seleccione una lección Previa..." required>
																...
																@foreach( $module->lessons as $less )
																	@if($less->id != $lesson->id )
																		<option value="{{ Hashids::encode($less->id) }}" {{ ($lesson->previous_id == $less->id ) ? 'selected' : '' }}>{{ $less->title }}</option>
																	@endif
																@endforeach
															</select>
														</div>
											...
											<div class="form-group">
												...
												<div class="col-md-7">
													<div class="input-group" id="defaultrange">
														<input type="text" class="form-control" name="daterange" value="{{ date('d/m/Y', strtotime($lesson->date_start)) . ' - ' . date('d/m/Y', strtotime($lesson->date_end))}}">
	...
	<script type="text/javascript">
		...
		window.history.pushState("", "", '/teachers/courses/show/{{ Hashids::encode($course->id) }}?section=lessons&action=editlesson&lesson_id={{ Hashids::encode($lesson->id) }}');
		document.title = 'Alice | {{ $course->title }} | {{ $lesson->title }} | Editar Lección';
